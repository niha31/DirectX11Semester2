...
HRESULT Application::Initialise(HINSTANCE hInstance, int nCmdShow)
{
	...
	_transform->SetRotation(XMConvertToRadians(90.0f), 0.0f, 0.0);
	_appearance->SetTextureRV(_pGroundTextureRV);
	_appearance->SetGeometry(planeGeometry);
	_appearance->SetMaterial(noSpecMaterial);

	GameObject * gameObject = new GameObject("Floor", _transform, _appearance);
	...
	for (auto i = 0; i < NUMBER_OF_CUBES; i++)
	{
		...
		_transform->SetPosition(-4.0f + (i * 2.0f), 0.5f, 10.0f);
		_appearance->SetTextureRV(_pTextureRV);
		_appearance->SetGeometry(cubeGeometry);
		_appearance->SetMaterial(shinyMaterial);
		gameObject = new GameObject("Cube", _transform, _appearance);
	...
	{
		_transform->SetScale(0.5f, 0.5f, 0.5f);
		_transform->SetPosition(0.0f, 5.0f, 10.0f);
		_appearance->SetTextureRV(_pTextureRV);
		_appearance->SetGeometry(cubeGeometry);
		_appearance->SetMaterial(shinyMaterial);

		ParticleSystem* particle = new ParticleSystem(_transform, _appearance);
...
void Application::Update()
{
	...
	if (GetKeyState('1') & 0x8000)
	{
		...
		_gameObjects.at(1)->GetParticleModel()->CanMove();
		_gameObjects.at(2)->GetParticleModel()->CantMove();
	}

	//_gameObjects[_selectedGameObject]->GetParticleModel()->Update(timeSinceStart);
	...
	for (auto particles:_particles)
	{
		if (particles->isDead())
		{
			particles->GetTransform()->SetPosition(0.0f, 5.0f, 10.0f);
		}
		particles->Update(timeSinceStart);
	}
	...
	for (int i = 0; i < _gameObjects.size(); i++)
	{
		...
		{
			if (_gameObjects.at(i)->GetParticleModel()->CollisionCheck(_gameObjects.at(j)->GetTransform()->GetPosition(), _gameObjects.at(j)->GetParticleModel()->GetBoundingSphere()) && i != j)
			{
				ResolveCollision(_gameObjects.at(i)->GetParticleModel(), _gameObjects.at(j)->GetParticleModel());
				_gameObjects.at(i)->GetParticleModel()->Update(timeSinceStart);
				_gameObjects.at(j)->GetParticleModel()->Update(timeSinceStart);		
			}
		}
	}
	...
	if (GetKeyState('2'))
	{
		_gameObjects.at(2)->GetTransform()->GetQuaternion()->addScaledVector(Vector(0.0f, 0.2f, 0.2f), 0.1f);
		XMStoreFloat4x4(_gameObjects.at(2)->GetTransform()->GetWorldMatrixPointer(), XMMatrixRotationQuaternion(*(_gameObjects.at(2)->GetTransform()->ReturnVectorQuaternion())) * XMMatrixScaling(_gameObjects.at(2)->GetTransform()->GetScale().x, _gameObjects.at(2)->GetTransform()->GetScale().y, _gameObjects.at(2)->GetTransform()->GetScale().z) * XMMatrixRotationY(timeSinceStart / 10) * XMMatrixTranslation(_gameObjects.at(2)->GetTransform()->GetPosition().x, _gameObjects.at(2)->GetTransform()->GetPosition().y, _gameObjects.at(2)->GetTransform()->GetPosition().z));
	}
	
	Draw();
}

void Application::ResolveCollision(ParticleModel* obj1, ParticleModel* obj2)
{
	...
	float coefficiet = 10.0f;
	Vector finalVelocityObj1;
	Vector finalVelocityObj2;
	Vector InitialVelocityObj1 = obj1->GetVelocity();
	Vector InitialVelocityObj2 = obj1->GetVelocity();

	float massObj1 = obj1->GetWeight();
	float massObj2 = obj2->GetWeight();

	finalVelocityObj1 = ((InitialVelocityObj1 * massObj1) + (InitialVelocityObj2 * massObj2) + ((InitialVelocityObj2 - InitialVelocityObj1) * (massObj2 * coefficiet))) / (massObj1 + massObj2) * -1;
	finalVelocityObj2 = ((InitialVelocityObj1 * massObj1) + (InitialVelocityObj2 * massObj2) + ((InitialVelocityObj1 - InitialVelocityObj2) * (massObj1 * coefficiet))) / (massObj1 + massObj2);

	obj1->SetVelocity(finalVelocityObj1);
	obj2->SetVelocity(finalVelocityObj2);
}
...
void Application::Draw()
{
	...
	for (auto gameObject : _gameObjects)
	{
		...
		//cb.World - XMMatrixTranspose(XMLoadFloat4x4(&gameObject->GetTransform()->ReturnModelMatrix()));
